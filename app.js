...
(function () {
  ...
  function readCcsSummary(eqId){
    const eqk = eqId || "NO_EQ";
    const docs = [
      readJSON(`nexus_${eqk}_ccs_vanguard_export`),
      readJSON(`nexus_${eqk}_ccs_two_tab_v1`),
      readJSON(`nexus_${eqk}_ccs_excel`),
      readJSON(`nexus_${eqk}_construction_check_sheet`)
    ].filter(Boolean);
    if (!docs.length) return null;
    const primary = docs[0] || {};
    return {
      raw: primary,
      signed: !!(primary.final || primary.finalSignoff || primary.signedOff || primary.complete === true || String(primary.status || "").toUpperCase() === "COMPLETE"),
      issueRows: Number(primary.issueRows || (primary.validation && Array.isArray(primary.validation.issues) ? primary.validation.issues.length : 0) || (Array.isArray(primary.issues) ? primary.issues.length : 0) || 0),
      status: String(primary.status || (primary.complete === true ? "COMPLETE" : "")).toUpperCase()
    };
  }
  ...
  function isCcsActuallyComplete(){
    try{
      const summary = readCcsSummary(eq || "NO_EQ");
      const signed = localStorage.getItem(ccsSignedKey()) === "1" || localStorage.getItem(stepKey("ccs")) === "1" || !!(summary && summary.signed);
      if (!summary || !signed) return false;
      if (summary.issueRows > 0) return false;
      if (summary.status && summary.status !== "COMPLETE") return false;
      return true;
    }catch(e){
      return false;
    }
